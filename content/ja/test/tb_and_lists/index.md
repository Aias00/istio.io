...
1. `kubectl` コマンドでアプリケーションをデプロイします：
   ...
   {{< warning >}}
    ...
    [手動サイドカーインジェクション](/ja/docs/setup/additional-setup/sidecar-injection/#manual-sidecar-injection) を使用している場合は、
    まず `istioctl kube-inject` コマンドで `bookinfo.yaml` ファイルを修正してからアプリケーションをデプロイしてください。
    詳細は `istioctl` の [リファレンスドキュメント](/ja/docs/reference/commands/istioctl/#istioctl-kube-inject) を参照してください。
